...
async function fetchApi<T>(path: string, timeout = 8000): Promise<T> {
  ...
  try {
    ...
    // 404 is a valid response (item not found), NOT an API failure — don't trip circuit breaker
    if (res.status === 404) {
    ...
    if (err instanceof Error && !err.message.startsWith('Not found:') && !err.message.startsWith('DofusDude API') && !err.message.startsWith('API JSON parse')) {
      breaker.recordFailure();
    }
    throw err;
  } finally {
